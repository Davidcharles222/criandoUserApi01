...
class UserController{// lembrar de exportar UserController no final da página
    ...
    async create(req, res){/*4º nessa promise criamos nossos users */
        ...
        if(email == undefined || email == '' || email == ' '){// validando, se email == undefined 
    ...
    async login(req, res){//29º campo de logar
        ...
        if(user == undefined){
            res.status(404)
            res.send("Digite email válido")
            return
        }

        if(user != undefined || user != '' || user != ' '){// se usuário for diferente de undefined, significa que usuário encontrado
